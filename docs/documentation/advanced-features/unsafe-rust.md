...
invalid programs. Although the code _might_ be okay, if the Rust compiler
...
_unsafe superpowers_, that you can’t in safe Rust. Those superpowers include
the ability to:
...
[“Calling an Unsafe Function orMethod.”](#calling-an-unsafe-function-or-method)<!-- ignore --> Another case is
...
> the name of this function. _Mangling_ is when a compiler changes the name
...
Until now, we’ve not talked about _global variables_, which Rust does support
