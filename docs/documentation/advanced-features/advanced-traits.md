...
overloading. _Operator overloading_ is customizing the behavior of an operator
...
mean unless you use _fully qualified syntax_. For example, the `Animal` trait
...
get around this restriction using the _newtype pattern_, which involves
...
the trait on the wrapper. _Newtype_ is a term that originates from the Haskell
