...
the `if` blocks, we can define a closure and store the _closure_ in a variable
...
_definition_ of an anonymous function, not the _resulting value_ of calling the
...
_memoization_ or _lazy evaluation_.
...
difference between these traits in the [“Capturing the Environment withClosures”](#capturing-the-environment-with-closures)<!-- ignore --> section; in
...
`value` will be `None`. When code using a `Cacher` asks for the _result_ of the
...
- `FnOnce` consumes the variables it captures from its enclosing scope, known
  as the closure’s _environment_. To consume the captured variables, the
