...
_src/lib.rs_ file looks like Listing 11-3.
...
on line 10 in the _src/lib.rs_ file. The next section lists just the names of
...
_src/lib.rs_ file and write some tests for it using the `assert!` macro.
...
what a value _will_ be, but we know what the value definitely _won’t_ be if our
...
You can’t use the `#[should_panic]` annotation on tests that use `Result<T, E>`. To assert that an operation returns an `Err` variant, _don’t_ use the
