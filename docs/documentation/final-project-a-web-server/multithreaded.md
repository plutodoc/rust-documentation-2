...
_/sleep_ and sleeping for 5 seconds</span>
...
`else if` after the `if` block to check for the request to _/sleep_. When that
...
that the requests to _/_ don’t have to wait for _/sleep_ to finish. But as we
...
Then create a new directory, _src/bin_, and move the binary crate rooted in
_src/main.rs_ into _src/bin/main.rs_. Doing so will make the library crate the
primary crate in the _hello_ directory; we can still run the binary in
_src/bin/main.rs_ using `cargo run`. After moving the _main.rs_ file, edit it
to bring the library crate in and bring `ThreadPool` into scope by adding the
following code to the top of _src/bin/main.rs_:
...
External code (like our server in _src/bin/main.rs_) doesn’t need to know the
...
In Chapter 16, you learned about _channels_—a simple way to communicate between
