...
value, only adding the new value if the key _doesn’t_ already have a value. Or
...
the value in `text`. The `or_insert` method returns a mutable reference (`&mut V`) to the value for the specified key. Here we store that mutable reference in
