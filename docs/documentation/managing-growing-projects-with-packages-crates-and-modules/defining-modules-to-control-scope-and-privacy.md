...
- **Start from the crate root**: When compiling a crate, the compiler first
  ...
  _src/main.rs_ for a binary crate).
...
- **Declaring submodules**: In any file other than the crate root that’s being
  compiled as part of the crate (for example, _src/garden.rs_), you can declare
...
- **The `use` keyword**: Within a scope, the `use` keyword creates shortcuts to
  ...
  `crate::garden::vegetables::Asparagus`, you can create a shortcut with `use crate::garden::vegetables::Asparagus;` and then only need to write `Asparagus`
...
And `pub mod vegetables;` means the code in _src/garden/vegetables.rs_ is
included too:
...
_Modules_ let us organize code within a crate into groups for readability and
easy reuse. Modules also control the _privacy_ of items, which is whether an
item can be used by outside code (_public_) or is an internal implementation
detail and not available for outside use (_private_).
...
Listing 7-1 into _src/lib.rs_ to define some modules and function signatures.
...
is the _child_ of module B and that module B is the _parent_ of module A.
